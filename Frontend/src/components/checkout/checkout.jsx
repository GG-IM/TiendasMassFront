...
export default function Checkout({ activeStep, setActiveStep, formData, setFormData, onChange }) {
  ...
  useEffect(() => {
    const cargarTarjetasUsuario = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:3000/api/tarjetas-usuario/usuario/${usuario.id}`, {
  ...
  useEffect(() => {
    const cargarDirecciones = async () => {
      if (!usuario?.id) return;
      try {
        const response = await fetch(`http://localhost:3000/api/direcciones/usuario/${usuario.id}`, {
          headers: getAuthHeaders(),
        });
        if (response.ok) {
