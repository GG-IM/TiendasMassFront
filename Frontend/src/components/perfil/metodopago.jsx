...
const Payments = () => {
  ...
  const fetchPaymentMethods = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:3000/api/tarjetas-usuario/usuario/${usuario.id}`, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`http://localhost:3000/api/tarjetas-usuario/${id}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      let res;
      if (isEditing) {
        res = await fetch(`http://localhost:3000/api/tarjetas-usuario/${form.id}`, {
          method: 'PUT',
          headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
      } else {
        res = await fetch('http://localhost:3000/api/tarjetas-usuario', {
          method: 'POST',
          headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
